// -*- c-basic-offset: 4 -*-
#ifndef CLICK_RADIXIPLOOKUPMPATHMT_HH
#define CLICK_RADIXIPLOOKUPMPATHMT_HH
...
RadixIPLookupMPathMT(MODE, ADDR1/MASK1 [GW11] OUT11 [[GW12] OUT12]..., ADDR2/MASK2 [GW21] OUT21 [[GW22] OUT22]..., ...)
...
Performs lockless, multithread-safe IP lookup using a radix trie. The first
level of the trie has 65536 buckets; each succeeding level has 16. The maximum
number of levels that will be traversed is thus 5.
...
class RadixIPLookupMPathMT : public IPRouteTableMPath { public:

    RadixIPLookupMPathMT() CLICK_COLD;
    ~RadixIPLookupMPathMT() CLICK_COLD;

    const char *class_name() const              { return "RadixIPLookupMPathMT"; }
  ...
  private:
    ...
    GWPortArr _lookup[255];
    atomic_uint32_t _lookup_size;

    mutable fast_rcu<Radix *> _radix;
    mutable per_thread<Radix *> _read_root;
