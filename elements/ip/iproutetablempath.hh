// -*- c-basic-offset: 4 -*-
#ifndef CLICK_IPROUTETABLEMPATH_HH
...
=item C<int B<add_route>(const IPRouteMPath& r, bool set, IPRouteMPath* old_route, ErrorHandler *errh)>
...
=item C<int B<remove_route>(const IPRouteMPath& r, IPRouteMPath* old_route, ErrorHandler *errh)>
...
struct IPRouteMPath {
    ...
    IPAddress addr;
    IPAddress mask;
    Vector<GWPort> gwports;
    int32_t extra;

    IPRouteMPath() : addr(), mask(), gwports() {}
    IPRouteMPath(IPAddress a, IPAddress m, Vector<GWPort> g)
                                : addr(a), mask(m), gwports(g) { }

    inline bool real() const    { return gwports.size() > 0; }
    inline void kill()          { addr = 0; mask = 0xFFFFFFFFU; gwports.clear(); }
    inline bool contains(IPAddress a) const;
    inline bool contains(const IPRouteMPath& r) const;
    inline bool mask_as_specific(IPAddress m) const;
    inline bool mask_as_specific(const IPRouteMPath& r) const;
    inline bool match(const IPRouteMPath& r) const;
...
class IPRouteTableMPath : public BatchElement { public:

    void* cast(const char*);
    int configure(Vector<String>&, ErrorHandler*) CLICK_COLD;
    void add_handlers() CLICK_COLD;

    int add_route(const IPRoute& route, bool allow_replace, IPRoute* replaced_route, ErrorHandler* errh);
    virtual int add_route(const IPRouteMPath& route, bool allow_replace, IPRouteMPath* replaced_route, ErrorHandler* errh);
    int remove_route(const IPRoute& route, IPRoute* removed_route, ErrorHandler* errh);
    virtual int remove_route(const IPRouteMPath& route, IPRouteMPath* removed_route, ErrorHandler* errh);
    virtual int lookup_route(IPAddress addr, IPAddress& gw) const;
    virtual int lookup_route(IPAddress addr, IPAddress& gw, uint32_t hash) const = 0;
  ...
  private:
    ...
    inline uint32_t calc_hash(Packet *p);
    ...
    inline int process(Packet *p);
};

inline StringAccum&
operator<<(StringAccum& sa, const IPRouteMPath& route)
{
    return route.unparse(sa, false);
}

inline bool
IPRouteMPath::contains(IPAddress a) const
{
    return a.matches_prefix(addr, mask);
}

inline bool
IPRouteMPath::contains(const IPRouteMPath& r) const
{
    return r.addr.matches_prefix(addr, mask) && r.mask.mask_as_specific(mask);
}

inline bool
IPRouteMPath::mask_as_specific(IPAddress m) const
{
    return mask.mask_as_specific(m);
}

inline bool
IPRouteMPath::mask_as_specific(const IPRouteMPath& r) const
{
    return mask.mask_as_specific(r.mask);
}

inline bool
IPRouteMPath::match(const IPRouteMPath& r) const
{
    if (addr != r.addr || mask != r.mask)
        return false;
    if (gwports.empty())
        return true;
    if (gwports.size() != r.gwports.size())
        return false;
    for (int j = 0; j < gwports.size(); j++)
        if (gwports[j].gw != r.gwports[j].gw || gwports[j].port != r.gwports[j].port)
            return false;
    return true;
}
